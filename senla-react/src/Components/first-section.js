  ...
  render() { 
    return (
      <section className="first-section">
        ...
        <div className="wrap">
            <input value={this.state.value} onChange={this.getVal} type="text" name="text" id="input-rep"/>
            <button onClick={this.setVal} onClick={this.getGitHubUser} id="search1">Async</button>
            <button id="search2">promis</button>
        </div>
        <div>
          <ul>
            { this.state.arr && (this.state.arr.map((item) => <li>Rep name: {item.name}</li>)) }
            { this.state.arr && (this.state.arr.map((item) => <li>Fullname: {item.full_name}</li>)) }
          </ul>
        </div>
      </section>
    )
